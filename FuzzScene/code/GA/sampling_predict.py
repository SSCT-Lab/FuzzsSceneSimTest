...
def predict(model_name,seed_number,q):
    ...
    x = tf.keras.layers.Dropout(0.5)(x)
    predictions = tf.keras.layers.Dense(2, activation='softmax')(x)  # softmax回归，10分类
...
def prenum(model_name,seed_number): 
    with open('../scenario_runner-0.9.13/_out/label_test.csv', 'r') as f:
        ...
        with open('/home/vangogh/software/FuzzScene/code/GA/sample_num_vgg.csv', 'a+', encoding='utf-8') as f:
