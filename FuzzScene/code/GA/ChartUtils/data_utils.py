...
def load_carla_test_data(path='', batch_size=32, shape=(100, 100)):
    ...
    with open(path, 'r') as f:
        ...
        for i, line in enumerate(f):
            if i == 0:
                continue
            xs.append(Constants.CARLA_RADAR_PNG_OUTPUT_PATH + line.split(',')[1])
...
def carla_load_steering_data(steering_log):
    df_steer = pd.read_csv(steering_log, usecols=['img_id', 'steering_angle_change'], index_col=False)
...
def carla_read_steerings(steering_log):
    ...
    with open(steering_log) as f:
        for line in f.readlines()[1:]:
            fields = line.split(",")
            timestamp, angle = int(str(fields[1])[:-4]), float(fields[3])
...
def carla_data_generator(frame_id, steering_log, image_folder, unique_list, gen_type='train',
    ...
    # print("sampling data from frame_id %d to %d" % (start, end))
    # print('frame_id len: ', len(frame_id))
    # print('steerings len: ', len(steerings))
